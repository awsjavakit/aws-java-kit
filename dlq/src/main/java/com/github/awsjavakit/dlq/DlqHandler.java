...
/**
 * This DLQ handler pushes the failed events to an S3 bucket through a firehose for automatically
 * organizing the events by time. The DLQ handler is agnostic to the kind of event that has failed.
 * It only stores the event to a bucket.
 * <p>
 * Example setup:
 * <pre>{@code
 *   SomeLambda:
 ...
 * <p>
 * This class cannot be instantiated on purpose. This will force developers to create an explicit
 * trace of the logic they are using in their code. Example implementation can be found in tests.
 */
public class DlqHandler implements RequestHandler<SQSEvent, Void> {
  ...
  public Void handleRequest(SQSEvent input, Context context) {
    ...
    Lists.partition(failedEvents, NUMBER_OF_GROUPS)
      .forEach(failedEventsHandlingService::handleFailedEvents);
    return null;
  }
